...
class MultiSetViewController: UITableViewController {
    ...
    private lazy var selection = LimitMultiSelection(inner: MultiSelection(container: self.controller, selectionContainerType: Multi.Set.self), limit: 5)
    
    override func viewDidLoad() {
        super.viewDidLoad()
        self.title = "Multi Selection - Set"
        self.navigationItem.rightBarButtonItem = UIBarButtonItem(title: "Done", style: .done, target: self, action: #selector(MultiSetViewController.doneButtnTapped(_:)))
    }
